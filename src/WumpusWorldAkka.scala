...
object Game {
  ...
  case class GameRunning(arrows: Int, room: Room) extends GameState {
    ...
    if (room == Empty) {
      println("Room is empty, game is running")
    }
    else if (room == WumpusInside) {
      println("Wumpus is inside the Room, games over.")
    }
    else if (room == BottomLessPit) {
      println("You've got in the Pit, games over.")
    }
    else if (room == ArrowInside) {
      println("You`ve been hit, game is still running")
    }
  }
